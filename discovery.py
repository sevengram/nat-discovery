...
def _initialize():
  ...
  for i in items:
    dictValToAttr.update({i[1]: i[0]})
  items = dictMsgTypeToVal.items()
  for i in items:
    dictValToMsgType.update({i[1]: i[0]})
...
def stun_test(sock, host, port, source_ip, source_port, send_data=""):
  ...
  while not recvCorr:
    ...
    tranid_match = tranid.upper() == binascii.b2a_hex(buf[4:20]).decode(
        'ascii').upper()
    if bind_resp_msg and tranid_match:
      ...
      while len_remain:
        ...
        if attr_type.decode('ascii') == MappedAddress:
        ...
        if attr_type.decode('ascii') == SourceAddress:
          port = int(binascii.b2a_hex(buf[base + 6:base + 8]), 16)
          ip = ".".join([
              str(int(
                  binascii.b2a_hex(buf[base + 8:base + 9]), 16
        ...
        if attr_type.decode('ascii') == ChangedAddress:
          port = int(binascii.b2a_hex(buf[base + 6:base + 8]), 16)
          ip = ".".join([
              str(int(
                  binascii.b2a_hex(buf[base + 8:base + 9]), 16
